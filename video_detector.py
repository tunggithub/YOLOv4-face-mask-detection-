...
def get_yolo_preds(net, input_vid_path, output_vid_path="output_video/yolo_output.avi", confidence_threshold=0.5, overlapping_threshold=0.3, write_output=False, show_display=True, labels = None):
    ...
    if write_output:
        ...
        out = cv2.VideoWriter(output_vid_path, cv2.VideoWriter_fourcc(
            *"mp4v"), cap.get(cv2.CAP_PROP_FPS), (frame.shape[1], frame.shape[0]), True)
...
yolo_config_path="yolov4-custom.cfg"
yolo_weights_path="yolov4-custom_best.weights"

input_vid_path = "testvideo1.mp4"
output_vid_path="outvideo.mp4"
...
get_yolo_preds(net,input_vid_path,output_vid_path,confidence_threshold,overlapping_threshhold,write_output,show_display,labels)